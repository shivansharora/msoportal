...
function CustomAutocomplete({ options, onChange, defaultValue }) {
	return (
		<Autocomplete
			...
			renderInput={props => <TextField label="Chief Complaint" {...props} fullWidth />}
		/>
	);
}

const EditMedicalProblem = props => {
    ...
    console.log(value)
		if (value === null) {
			...
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				...
				axios.get(`/get_asscociated_symptoms_by_id_dropdown/${value.id}`, { headers: { Authorization: token } }).then(response => {
	...
	useEffect(() => {
		...
		const fetchMedicalProblems = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_medical_problems_dropdown', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
		...
		const fetchDurationType = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_duration_type_list', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
		...
		const fetchProgressionList = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_progression_list', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
		...
		const fetchOccurenceList = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_occurrence_list', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
		...
		const fetchCurrentStatus = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_medical_problem_current_status_list', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
		...
		const fetchVerbs = () => {
			if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
				let token = "Bearer " + localStorage.getItem("jwt");
				axios.get('/get_verbs_dropdown', { headers: { Authorization: token } }).then(response => {
					if (mounted) {
	...
	useEffect(()=>{
		if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
			let token = "Bearer " + localStorage.getItem("jwt");
			axios.get(`patient_medical_problems/${props.match.params.id}`, { headers: { Authorization: token } }).then(response => {
					setMedicalData(response.data);
					setOnset(response.data.occurrence.key)
					setProgression(response.data.progression.key)
					setDuration(response.data.duration)
					setDurationType(response.data.duration_type.key)
					setMed(response.data.complaint_id)
					let langString = response.data.aggravating_factor
					// let langArray = langString.split(',');
					// setAggravatingFactor(response.data.aggravating_factor)
					console.log(response.data.aggravating_factor)


					// setOnsetHindi(response.data.occurrence)
					// if(response)
	...
	function medicalProblem() {
		...
		if (localStorage.getItem("jwt") != '' || localStorage.getItem("jwt") !== undefined) {
			let token = "Bearer " + localStorage.getItem("jwt");
			axios.post(`/patient_medical_problems `, { patient_medical_problem: patient_medical_problem }, { headers: { Authorization: token } }).then(response => {
						...
						<CardBody>
						    ...
						    //    options={options.map(e => ({ label: e.phrase_english, value: e.id}))}
								...
								/> */}
									...
									{medicalData.progression !== undefined && (
									<Grid item xs={12} sm={6} md={6} >
										<TextField style={{ minWidth: 275 }}
											id="progression"
											select
									...
									<Grid item xs={12} sm={6} md={6} >
										<div style={{ marginLeft: -7, width: 290 }}>
											...
											<MultiSelect
												...
												options={verbs.map(e => ({ label: e.phrase_english, value: e.id, key:e.phrase_hindi }))}
									...
									<Grid item xs={12} sm={6} md={6} >
										<div style={{ marginLeft: -7, width: 290 }}>
											...
											<MultiSelect
												...
												getOptionLabel={option => option.phrase_english}
												options={assoSymtom.map(e => ({ label: e.phrase_english, value: e.id, key:e.phrase_hindi }))}
									...
									<CardFooter style={{ float: 'right' }}>
										...
										<Button style={{ width: 72 }}
											...
											to={`/patient_list/${props.match.params.id}`}
										>Cancel</Button>
									</CardFooter>
								</Grid>
							</form>
...
export default EditMedicalProblem;
