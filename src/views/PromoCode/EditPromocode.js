...
const EditPromocode = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchPromo = () => {
      if (
        ...
        axios
          .get(`/promo_codes/${props.match.params.id} `, { headers: { Authorization: token } })
          .then((response) => {
            if (mounted) {
  ...
  useEffect(() => {
    if (promo.attributes !== undefined) {
      ...
      setTimeout(() => {
        ...
        setValue("discount", promo.attributes.discount || "");
        setValue(
          "max_usage",
          promo.attributes.max_usage || ""
        );
  ...
  const onSubmit = (data) => {
    ...
    if (
      ...
      axios
        .put(
          `/promo_codes/${props.match.params.id}`,
